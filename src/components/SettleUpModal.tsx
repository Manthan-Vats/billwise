...
import { Group, SimplifiedDebt, Settlement, Expense } from '../types';
import { formatCurrency } from '../utils/formatters';
import { isValidSettlement } from '../utils/settlementValidation';
...
export const SettleUpModal: React.FC<SettleUpModalProps> = ({
  ...
  const handleSettle = () => {
    ...
    const amount = customAmount ? parseFloat(customAmount) : selectedDebt.amount;
    ...
    onAddSettlement({
      ...
      amount,
      currency: 'USD',
      description: notes.trim() || `Settlement: ${getMemberName(selectedDebt.fromMemberId)} → ${getMemberName(selectedDebt.toMemberId)}`,
    });
      ...
      <div className="bg-white rounded-2xl p-6 w-full max-w-lg max-h-[80vh] overflow-y-auto">
        ...
        {simplifiedDebts.length === 0 ? (
          <div className="text-center py-8">
            ...
            <h3 className="text-lg font-medium text-slate-800 mb-2">All Settled!</h3>
            <p className="text-slate-600">No outstanding debts to settle.</p>
          </div>
        ) : (
          <div className="space-y-6">
            ...
            <div>
              ...
              {maxSettlementAmount > 0 && (
                <p className="text-xs text-slate-500 mb-3">
                  Maximum settlement amount: {formatCurrency(maxSettlementAmount, 'USD')}
                </p>
              )}
                          ...
                          <div className="flex flex-col items-end">
                            ...
                            {debt.amount > maxSettlementAmount && (
                              <span className="text-xs text-amber-600">
                                Max: {formatCurrency(maxSettlementAmount, 'USD')}
                              </span>
                            )}
                          </div>
                        </div>
              ...
              <>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {['cash', 'bank-transfer', 'digital-wallet'].map((method) => (
                      <button
                        key={method}
                        type="button"
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      type="text"
                      className="w-full pl-6 pr-4 py-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                      placeholder={selectedDebt ? formatCurrency(selectedDebt.amount, 'USD') : '0.00'}
                      value={customAmount}
                      onChange={(e) => {
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows={3}
                    className="w-full px-4 py-2.5 rounded-xl border border-slate-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 transition-all resize-none"
                  />
                </div>
              </>
            )}
            ...
            <div className="flex space-x-3 pt-4 border-t border-slate-200">
              ...
              <button
                ...
                title={!selectedDebt ? 'Select a debt to settle' : !isSettlementValid() ? 'Enter a valid amount' : ''}
              >
                <span>Record Payment of {formatCurrency(getSettlementAmount(), 'USD')}</span>
              </button>
            </div>
          </div>
        )}
